...
const ESCAPE_SIMPLE: CharClass = CharClass::EMPTY
    .with_byte(b'"').with_byte(b'\\').with_byte(b'/')
    .with_byte(b'b').with_byte(b'f').with_byte(b'n')
    .with_byte(b'r').with_byte(b't');
...
fn emit_shared_rules(g: &mut GrammarBuilder) {
    ...
    g.parser_rule("value", |g| {
        g.byte_dispatch(vec![                                     // auto: ws → dispatch
            (CharClass::EMPTY.with_byte(b'"'), Box::new(|g: &mut GrammarBuilder| {
                g.call("string");
            })),
            (CharClass::EMPTY.with_byte(b'{'), Box::new(|g: &mut GrammarBuilder| {
                g.call("object");
            ...
            (classes::DIGIT.with_byte(b'-'),   Box::new(|g: &mut GrammarBuilder| {
                g.call("number");
            })),
            (CharClass::EMPTY.with_byte(b't'), Box::new(|g: &mut GrammarBuilder| {
                g.token(TOK_TRUE,  |g| { g.literal(b"true"); });
            })),
            (CharClass::EMPTY.with_byte(b'f'), Box::new(|g: &mut GrammarBuilder| {
                g.token(TOK_FALSE, |g| { g.literal(b"false"); });
            })),
            (CharClass::EMPTY.with_byte(b'n'), Box::new(|g: &mut GrammarBuilder| {
                g.token(TOK_NULL,  |g| { g.literal(b"null"); });
            })),
        ], None);
    });
    ...
    g.parser_rule("member", |g| {
        g.node(NODE_MEMBER, |g| {
            ...
            g.token(TOK_COLON, |g| { g.byte(b':'); });         // auto: ws + :
            g.call("value");                              // auto: ws + value
        });
    });
    ...
    g.parser_rule("array", |g| {
        g.node(NODE_ARRAY, |g| {
            g.token(TOK_LBRACKET, |g| { g.byte(b'['); });          // auto: ws + [
            g.optional(|g| {
                g.call("value");                              // auto: ws + value
                g.zero_or_more(|g| {
                    g.token(TOK_COMMA, |g| { g.byte(b','); });      // auto: ws + ,
            ...
            g.token(TOK_RBRACKET, |g| { g.byte(b']'); });          // auto: ws + ]
        });
    });
    ...
    g.lexer_rule("escape", |g| {
        g.byte(b'\\');
        g.choice(
            |g| { g.class(ESCAPE_SIMPLE); },
            |g| {
                g.byte(b'u');
                g.repeat(4u32, |g| { g.class(classes::HEX_DIGIT); });
            },
        );
    });
    ...
    g.lexer_rule("ws", |g| {
        g.zero_or_more(|g| {
            g.choice(
                ...
                |g| { g.choice(
                    |g| { g.call("line_comment"); },
                    |g| { g.call("block_comment"); },
                ); },
            );
        });
    });
...
fn preview(s: &str) -> String {
    let s = s.replace('\n', "↵").replace('\r', "");
    if s.len() <= 24 { s } else { format!("{}…", &s[..21]) }
}

fn print_tree(node: &SyntaxNode, indent: usize) {
    let r = node.text_range();
    println!("{}{} ({}..{})", "  ".repeat(indent), kind_name(node.kind()), r.start, r.end);
    for child in node.children() {
        match child {
            ...
            SyntaxElement::Token(t) => {
                ...
                println!("{}  {} {} {:?} ({}..{})",
                    "  ".repeat(indent), glyph,
                    kind_name(t.kind()), preview(t.text()), r.start, r.end);
            }
        }
    }
}
...
fn string_content(tok: &SyntaxToken) -> &str {
    let t = tok.text();
    if t.len() >= 2 && t.starts_with('"') && t.ends_with('"') { &t[1..t.len()-1] } else { t }
}

fn object_members(obj: &SyntaxNode) -> Vec<(String, String)> {
    obj.find_all_nodes(NODE_MEMBER).into_iter().map(|member| {
        let sem: Vec<SyntaxToken> = member.non_trivia_tokens().collect();
        let key = sem.first().map(|t| string_content(t).to_string()).unwrap_or_default();
        let scalar_value = sem.get(2).map(|t| t.text().to_string());
        let node_value   = member.child_nodes().last().map(|n| format!("<{}>", kind_name(n.kind())));
...
fn check(pass: &mut usize, fail: &mut usize, ok: bool, label: &str) {
    if ok { *pass += 1; println!("  [✓] {label}"); }
    else  { *fail += 1; println!("  [✗] {label}"); }
}
...
fn main() {
  ...
  "active": true  // currently enrolled
    ...
    let block = all_trivia.iter().find(|t| t.kind() == TRIVIA_BLOCK_COMMENT).unwrap();
    c(block.text().starts_with("/*") && block.text().ends_with("*/"), "block comment delimiters correct");
    c(block.text().contains("Higher is better"), "block comment body preserved verbatim");

    let members_c = root_c.find_node(NODE_OBJECT).map(|o| object_members(&o)).unwrap_or_default();
    c(members_c == doc_members, "JSONC semantic members match JSON semantic members");
    ...
    c(engine.parse(&json_g,  plain).is_ok(),              "plain JSON: accepted by JSON grammar");
    c(engine.parse(&jsonc_g, plain).is_ok(),              "plain JSON: accepted by JSONC grammar");
    c(engine.parse(&json_g,  with_line_comment).is_err(), "// comment: rejected by JSON grammar");
    c(engine.parse(&jsonc_g, with_line_comment).is_ok(),  "// comment: accepted by JSONC grammar");
    c(engine.parse(&json_g,  with_block_comment).is_err(),"/* */ comment: rejected by JSON grammar");
    c(engine.parse(&jsonc_g, with_block_comment).is_ok(), "/* */ comment: accepted by JSONC grammar");
    ...
    // We can verify it works by building a tiny ad-hoc grammar:
    {
        ...
        g2.parser_rule("path", |g| {
            g.node(1, |g| {
                g.token(10, |g| {           // ident: auto ws + ident
                    g.one_or_more(|g| { g.class(classes::IDENT_CONT); });
                });
                g.no_skip(|g| {            // protect "::": no ws injection inside
                    g.token(11, |g| { g.literal(b"::"); });
                });
                g.token(10, |g| {           // ident: auto ws + ident
                    g.one_or_more(|g| { g.class(classes::IDENT_CONT); });
                });
        ...
        g2.lexer_rule("ws", |g| {
            g.trivia(30, |g| { g.zero_or_more(|g| { g.class(classes::WHITESPACE); }); });
        });

        let bg2 = g2.finish().unwrap();
        let gr2 = bg2.as_graph();

        c(engine.parse(&gr2, b"foo::bar").is_ok(),    "foo::bar     accepted");
        c(engine.parse(&gr2, b"  foo :: bar").is_err(),"foo :: bar   rejected (space in ::)");
        c(engine.parse(&gr2, b"  foo::bar  ").is_ok(), "leading/trailing ws accepted (skip() around path)");
    }
    ...
    for &(input, json_ok, jsonc_ok) in cases {
        ...
        println!("   [{mark}] {:<45}  {:5}  {:5}",
            if label.len() > 44 { &label[..44] } else { label },
            if got_json  { "✓" } else { "✗" },
            if got_jsonc { "✓" } else { "✗" });
        if ok { table_pass += 1; } else { table_fail += 1; }
    }

    if table_fail == 0 { c(true,  &format!("all {table_pass} table cases correct")); }
    else               { c(false, &format!("{table_fail} table cases incorrect"));  }

    // ── Summary ───────────────────────────────────────────────────────────────
    println!();
    println!("══ Results: {pass} passed, {fail} failed out of {}. ═════════",
             pass + fail);
