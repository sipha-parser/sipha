...
fn print_tree(node: &SyntaxNode, indent: usize) {
    ...
    println!("{}Node({}) {}..{}", pad, kind_name(node.kind()), range.start, range.end);
    for child in node.children() {
        match child {
            ...
            SyntaxElement::Token(t) => {
                ...
                println!(
                    "{}  Token({}){}  {:?}  {}..{}",
                    pad, kind_name(t.kind()), trivia_mark,
                    t.text(), t.offset(), t.offset() + t.text_len()
                );
            }
        }
    }
...
fn main() {
    ...
    println!("─── Tree for {:?} ───", std::str::from_utf8(src1).unwrap().trim());
    ...
    println!("─── Tree for {} statements ───", root2.find_all_nodes(NODE_STMT).len());
    print_tree(&root2, 0);
    println!();
    ...
    c(kw0.kind() == TOK_KEYWORD, "first token of stmt[0] is Keyword");
    c(kw0.text() == "let",       "Keyword text is 'let' (no source buffer needed)");
    ...
    c(tok_at_3.is_trivia(), "token_at_offset(3) is trivia (whitespace)");

    // semantic_token_at_offset skips trivia.
    let sem = root2.semantic_token_at_offset(3);
    c(sem.is_none(), "semantic_token_at_offset(3) = None (trivia position)");
    ...
    c(groups.len() == 3, &format!("stmt[0] has 3 token groups (got {})", groups.len()));
    ...
    c(g0.trailing.len() == 1,         "group[0] has 1 trailing trivia (space)");
    c(g0.trailing[0].text() == " ",   "group[0] trailing trivia text = ' '");

    // Group 1: ident "foo"
    let g1 = &groups[1];
    c(g1.token.kind() == TOK_IDENT, "group[1] is Ident");
    c(g1.leading.is_empty(),        "group[1] has no leading trivia");
    c(g1.trailing.is_empty(),       "group[1] has no trailing trivia (no space before ';')");

    // Group 2: semicolon ";"
    let g2 = &groups[2];
    c(g2.token.kind() == TOK_SEMICOLON, "group[2] is Semicolon");
    c(g2.trailing.len() == 1,           "group[2] has trailing newline trivia");
    c(g2.trailing[0].text() == "\n",    "group[2] trailing trivia = '\\n'");

    // full_range covers everything including trivia.
    let stmt0_full = stmts[0].text_range();
    c(stmt0_full.start == 0 && stmt0_full.end == 9,
      &format!("stmt[0] range = 0..9, got {}..{}", stmt0_full.start, stmt0_full.end));
    ...
    c(kw_group.full_text() == "let ", &format!("kw full_text = {:?}", kw_group.full_text()));
    ...
    c(sc_group.full_text().ends_with('\n'),
      &format!("semicolon full_text ends with newline: {:?}", sc_group.full_text()));

    println!();
    println!("─── descendant_semantic_tokens ───");

    let all_sem: Vec<_> = root2.descendant_semantic_tokens();
    c(all_sem.len() == 9, // 3 stmts × (keyword + ident + semicolon)
    ...
    c(idents[0].text() == "foo" && idents[1].text() == "bar" && idents[2].text() == "baz",
    ...
    // Missing semicolon.
    let bad1 = b"let foo\n";
    c(engine.parse(&graph, bad1).is_err(), "missing semicolon → parse error");

    // Empty input: valid (zero statements).
    let empty = b"";
    let out_empty = engine.parse(&graph, empty).expect("empty input should succeed");
    let root_empty = out_empty.syntax_root(empty).expect("empty tree");
    c(root_empty.find_all_nodes(NODE_STMT).is_empty(), "empty input → 0 stmts");
    c(root_empty.collect_text() == "", "empty input → collect_text = ''");

    // Whitespace-only input.
    let ws_only = b"  \n  ";
    let out_ws = engine.parse(&graph, ws_only).expect("ws-only should succeed");
    let root_ws = out_ws.syntax_root(ws_only).expect("ws tree");
    c(root_ws.find_all_nodes(NODE_STMT).is_empty(), "ws-only input → 0 stmts");
    c(root_ws.collect_text() == "  \n  ", "ws text preserved in tree");
    ...
    let first_kw = owned_root.descendant_semantic_tokens()
        .into_iter().find(|t| t.kind() == TOK_KEYWORD).unwrap();
    c(first_kw.text() == "let", "GreenToken text survives drop of source buffer");
