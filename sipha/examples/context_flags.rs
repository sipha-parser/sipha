...
fn main() {
    ...
    for (i, e) in graph_data.flag_mask_data.iter().enumerate() {
        println!(
            "  entry[{i}]: word={} set={:#066b} clear={:#066b}",
            e.word, e.set_bits, e.clear_bits
        );
    }
    println!(
        "  {} mask(s) interned, {} total word entries",
    ...
    let mut c = |src, ctx: &ParseContext, ok, desc| {
        if check(&mut engine, &graph, src, ctx, ok, desc) {
            pass += 1
        } else {
            fail += 1
        }
    ...
    c(
        "fn foo() { loop { break; } return; }",
        &no_ctx,
        true,
        "break in loop, return in fn",
    );
    c(
    ...
    c(
        "fn foo() { loop { return 0; } }",
        &no_ctx,
        true,
        "return in loop inside fn",
    );
    c(
        "loop { fn inner() { 0; } break; }",
        &no_ctx,
        true,
        "IN_LOOP restored after inner fn",
    );
    ...
    c(
        ...
        &in_fn_ctx,
        true,
        "return with caller-supplied IN_FN context",
    );
    ...
    assert!(
        complex_ctx.has(FLAG_STRICT),
        "FLAG_STRICT should still be set in context"
    );
    assert!(
        complex_ctx.has(FLAG_ASYNC),
        "FLAG_ASYNC should still be set in context"
    );
