...
fn check(
    ...
    if ok {
        println!(
            "  [{sym}] {desc:<58} → OK  ({} bytes)",
            result.unwrap().consumed
        );
    } else {
...
fn main() {
    ...
    macro_rules! c {
        ($g:expr, $inp:expr, $ok:expr, $desc:expr) => {{
            let bg = $g;
            let gr = bg.as_graph();
            if check(&mut engine, &gr, $inp, $ok, $desc) {
                pass += 1
            } else {
                fail += 1
            }
    ...
    // Invalid UTF-8: bare 0x80 continuation byte
    {
        ...
        println!(
            "  [{sym}] {:<58} → {}",
            "invalid UTF-8 (0x80 continuation as lead)",
            if ok { "OK (wrong!)" } else { "ERR (correct)" }
        );
        if ok {
            fail += 1
        } else {
    ...
    c!(
        ...
        "$",
        false,
        "U+0024 $ does not match €"
    );
    c!(
        grammar_exact_char('€'),
        "£",
        false,
        "U+00A3 £ does not match €"
    );
    c!(grammar_exact_char('α'), "α", true, "U+03B1 α matches α");
    c!(
        grammar_exact_char('α'),
        "Α",
        false,
        "U+0391 Α (capital) does not match α"
    );
    c!(
        grammar_exact_char('🦀'),
        "🦀",
    ...
    c!(
        ...
        "Α",
        false,
        "Α = U+0391 below range"
    );
    c!(
        grammar_char_range('α', 'ω'),
        "ϊ",
        false,
        "ϊ = U+03CA above range"
    );
    c!(
        grammar_char_range('α', 'ω'),
        "A",
    ...
    c!(
        grammar_char_range('\u{1F600}', '\u{1F64F}'),
        "😀",
        true,
        "😀 U+1F600 in smiley block"
    );
    c!(
        grammar_char_range('\u{1F600}', '\u{1F64F}'),
        "🙏",
        true,
        "🙏 U+1F64F in smiley block (upper)"
    );
    c!(
        grammar_char_range('\u{1F600}', '\u{1F64F}'),
        "🎉",
        false,
        "🎉 U+1F389 not in smiley block"
    );
    ...
    c!(
        grammar_any_char_repeat(),
        "",
        false,
        "empty (need at least 1)"
    );
    ...
    c!(
        ...
        "xxxx",
        false,
        "Exact(3): 4 (trailing rejected by EOF)"
    );
    ...
    c!(
        ...
        "aaaaaa",
        false,
        "Between(2,5): 6 (> max, trailing rejected)"
    );
    ...
    c!(
        ...
        "aaa",
        true,
        "Between(3,3) == Exact(3): 3"
    );
    c!(
        grammar_between(3, 3),
        "aa",
        false,
        "Between(3,3) == Exact(3): 2"
    );
    ...
    c!(
        ...
        "",
        true,
        "Between(0,3) == AtMost(3): 0"
    );
    c!(
        grammar_between(0, 3),
        "aaa",
        true,
        "Between(0,3) == AtMost(3): 3"
    );
    c!(
        grammar_between(0, 3),
        "aaaa",
        false,
        "Between(0,3) == AtMost(3): 4 (rejected)"
    );
    ...
    macro_rules! ct {
        ($inp:expr, $ok:expr, $desc:expr) => {{
            if check(&mut engine, &tag_g, $inp, $ok, $desc) {
                pass += 1
            } else {
                fail += 1
            }
    ...
    ct!(
        "[🎸🥁🎺🎻🎹:jazz]",
        false,
        "five emoji → ':' not found where expected"
    );
    ...
    println!();
    println!(
        "─── Between(2,5) expands to {} instructions: ───",
        {
