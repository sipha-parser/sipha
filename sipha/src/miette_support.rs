...
impl MietteParseDiagnostic {
    ...
    /// `rule_names` is the grammar's rule names for [`Expected::Rule`] display.
    /// `expected_labels` is for [`Expected::Label`] from [`expect_label`](crate::builder::GrammarBuilder::expect_label).
    pub fn new(
        ...
        let expected_msg = {
            let items: Vec<String> = diagnostic
                ...
                .collect();
            let list = crate::error::format_expected_list(&items);
            if list == "nothing" {
