...
pub fn emit_rust(graph: &BuiltGraph, out: &mut String) -> std::fmt::Result {
    ...
    writeln!(out)?;
    writeln!(out, "use sipha::insn::{{Insn, LiteralTable, FlagMaskTable, ParseGraph}};")?;
    ...
    for (i, b) in graph.literal_data.iter().enumerate() {
        write!(out, "0x{b:02X},")?;
        if i % 16 == 15 { write!(out, "\n    ")?; }
    }
    writeln!(out, "\n];")?;
    writeln!(out, "static LIT_OFFSETS: &[u32] = &[")?;
    write!(out, "    ")?;
    for o in &graph.literal_offsets { write!(out, "{o},")?; }
    writeln!(out, "\n];")?;
    writeln!(out)?;
    ...
    write!(out, "    ")?;
    for o in &graph.flag_mask_offsets { write!(out, "{o},")?; }
    writeln!(out, "\n];")?;
    writeln!(out)?;
    ...
    if !graph.jump_tables.is_empty() {
        ...
        for (ti, table) in graph.jump_tables.iter().enumerate() {
            ...
            for chunk in table.chunks(8) {
                write!(out, "        ")?;
                for &v in chunk {
                    if v == u32::MAX { write!(out, "u32::MAX, ")?; }
                    else             { write!(out, "{v}_u32, ")?; }
                }
                writeln!(out)?;
            }
            writeln!(out, "    ],")?;
    ...
    for n in &graph.rule_names { writeln!(out, "    \"{n}\",")?; }
    writeln!(out, "];")?;
...
fn emit_insn(out: &mut String, insn: &Insn) -> std::fmt::Result {
    match insn {
        ...
        Insn::EndOfInput { on_fail } =>
            write!(out, "Insn::EndOfInput {{ on_fail: {on_fail} }}"),
        Insn::AnyChar   { on_fail } =>
            write!(out, "Insn::AnyChar {{ on_fail: {on_fail} }}"),
        Insn::Char      { codepoint, on_fail } =>
            write!(out, "Insn::Char {{ codepoint: 0x{codepoint:06X}_u32, on_fail: {on_fail} }}"),
        Insn::CharRange { lo, hi, on_fail } =>
            write!(out, "Insn::CharRange {{ lo: 0x{lo:06X}_u32, hi: 0x{hi:06X}_u32, on_fail: {on_fail} }}"),
        ...
        Insn::ByteDispatch  { table_id } => write!(out, "Insn::ByteDispatch {{ table_id: {table_id} }}"),
        Insn::IfFlag    { flag_id, on_fail } =>
            write!(out, "Insn::IfFlag {{ flag_id: {flag_id}, on_fail: {on_fail} }}"),
        Insn::IfNotFlag { flag_id, on_fail } =>
            write!(out, "Insn::IfNotFlag {{ flag_id: {flag_id}, on_fail: {on_fail} }}"),
