#[cfg(feature = "query")]
use crate::syntax::{SyntaxNode, SyntaxElement, SyntaxKind};
...
impl<K: SyntaxKind> QueryBuilder<K> {
    ...
    {
        self.predicates.push(NodePredicate::Custom(Box::new(predicate)));
        self
    }
    ...
    pub fn find_first(&self, root: &SyntaxNode<K>) -> Option<SyntaxNode<K>> {
        root.descendants().find(|node| self.predicates.iter().all(|p| match p {
            NodePredicate::Kind(k) => node.kind() == *k,
            NodePredicate::Custom(f) => f(node),
        }))
    }
...
impl<K: SyntaxKind> SyntaxNode<K> {
    ...
    where
        F: Fn(&Self) -> bool + 'static,
    {
        QueryBuilder::new().with_predicate(predicate).find_first(self)
    }
    ...
    #[must_use]
    pub fn query(&self) -> QueryBuilder<K> {
        QueryBuilder::new()
    }
}
...
mod tests {
    ...
    fn test_find_matching() {
        let tree = build_test_tree();
        let results = tree.find_matching(|node| {
            matches!(node.kind(), TestKind::Expr | TestKind::Term)
        });
    ...
    fn test_xpath_query_parse_simple() {
        let query = XPathQuery::<TestKind>::parse("/Expr", |s| {
            match s {
    ...
    fn test_xpath_query_parse_descendant() {
        let query = XPathQuery::<TestKind>::parse("//Number", |s| {
            match s {
    ...
    fn test_xpath_query_execute_descendant() {
        ...
        let query = XPathQuery::<TestKind>::parse("//Term", |s| {
            match s {
                "Term" => Some(TestKind::Term),
                "Expr" => Some(TestKind::Expr),
                _ => None,
