        ...
        for i in 0..min_len {
            match (&self_children[i], &other_children[i]) {
                ...
                (
                    ...
                    if self_token.kind() != other_token.kind() || self_token.text() != other_token.text() {
                        diffs.push(TreeDiff::KindMismatch {
...
mod tests {
    use super::*;
    use crate::syntax::{SyntaxKind, GreenNodeBuilder, SyntaxNode};
    ...
    fn test_tree_diff_different_kinds() {
        ...
        let diffs = tree1.diff(&tree2);
        assert!(!diffs.is_empty());
        assert!(diffs.iter().any(|d| matches!(d, TreeDiff::KindMismatch { .. })));
    }
    ...
    fn test_tree_diff_different_child_count() {
        ...
        let diffs = tree1.diff(&tree2);
        assert!(!diffs.is_empty());
        assert!(diffs.iter().any(|d| matches!(d, TreeDiff::ChildCountMismatch { .. })));
    }
