    ...
    N: NonTerminal,
{
    ...
    fn build_table_entry_ll1(
        ...
        if let Expr::Choice(alternatives) = expr {
            for (alt_idx, alt) in alternatives.iter().enumerate() {
                ...
                if nullable
                    && let Some(follow) = follow_sets.get(lhs) {
                    for token in follow {
                        let key = (lhs.clone(), LookaheadKey::Token(token.clone()));
        ...
        if let Expr::Choice(alternatives) = expr {
            for (alt_idx, alt) in alternatives.iter().enumerate() {
                ...
                for seq in first_k {
                    ...
                    if table.insert(key, alt_idx).is_some() {
                        return Err(format!("Conflict in parsing table for rule {lhs:?} (LL({k}))"));
                    }
                }
    ...
    fn compute_first_k(
        ...
        Self::compute_first_k_impl(grammar, expr, first_sets, k, &mut result, &mut visited, &mut SmallVec::new());
        result
    }
        ...
        match expr {
            ...
            Expr::Rule(n) => {
                if visited.insert(n.clone())
                    ...
                    Self::compute_first_k_impl(grammar, &rule.rhs, first_sets, k, result, visited, current);
                }
            }
            Expr::Seq(exprs) => {
                for e in exprs {
    ...
    pub fn get(&self, nt: &N, lookahead: &[T]) -> Option<usize> {
        if self.k == 1 {
            ...
            let key = lookahead.first()
                .map_or((nt.clone(), LookaheadKey::Eof), |t| (nt.clone(), LookaheadKey::Token(t.clone())));
