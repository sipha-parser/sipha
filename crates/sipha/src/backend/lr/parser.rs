...
use crate::backend::lr::{table::LrParsingTable, config::LrConfig, state::LrParserState};
    ...
    N: NonTerminal,
{
    ...
    let mut stack: Vec<(usize, Vec<std::sync::Arc<GreenNode<T::Kind>>>)> = Vec::new();
            ...
            crate::backend::lr::table::Action::Reduce(prod_idx) => {
                if let Some(production) = table.get_production(prod_idx) {
                    ...
                    if let Some(goto_state) = table.get_goto(state_after_pop, &production.lhs) {
                        // Create node for LHS
                        let lhs_kind = production.lhs.to_syntax_kind()
